...
async function sendEmail({ to, subject, html }) {
  /*
    Replace this function body with your chosen email provider.

    Example expected behaviour:
    - send from EMAIL_FROM
    - send to recipient
    - use subject and html
    - throw an error if sending fails

    Do not call SMTP or email APIs from React frontend.
  */

  if (!EMAIL_API_KEY) {
    throw new Error(
      "EMAIL_API_KEY is not configured. Add your email provider API key in Vercel environment variables."
    );
  }

  // Provider integration goes here.
  // For now, this intentionally fails clearly until the provider is added.
  throw new Error(
    "Email provider integration has not been connected inside sendEmail()."
  );
}
